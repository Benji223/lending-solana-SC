...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    // get the sol price and usdc price
    ...
    if health_factor >= 1.0 {
        return Err(ErrorCode::HealthFactorAboveOne.into());
    }
    ...
    // calculation of what the liquidation amount can be 
    let liquidation_amount = total_borrowed.checked_mul(borrowed_bank.liquidation_close_factor).unwrap();

    // Transfer the borrowed tokens from the liquidator to the bank's treasury
    token_interface::transfer_checked(cpi_ctx, liquidation_amount, decimals)?;


    // transfering to the liquidator the collateral from the bank
    ...
    // Create the CPI context with the transfer accounts and signer seeds
