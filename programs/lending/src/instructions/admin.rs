...
pub struct InitBank<'info> {
    ...
    #[account(
        init,
        space = 8 + Bank::INIT_SPACE, // Bank account size
        payer = signer,
...
pub struct InitUser<'info> {
    ...
    #[account(
        init,
        payer = signer,
        space = 8 + User::INIT_SPACE, // User account size
...
pub fn process_init_bank(
    ...
    bank.interest_rate = 0.05 as u64; // Example interest rate of 5%

    Ok(())
}

pub fn process_init_user(ctx: Context<InitUser>,
    usdc_address: Pubkey
) -> Result<()> {
